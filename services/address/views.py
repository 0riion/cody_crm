...
from .serializers import AddressSerializer, AddressListSerializer, AddressRetrieveSerializer
from libs.request_event import camel_to_snake_dict, snake_to_camel_dict


class AddressView(viewsets.GenericViewSet):
    model = Address
    queryset = None
    ...
    def list(self, request):
        try:
            ...
            if zipcode:
                filter_request['zipcode'] = zipcode
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            serializer = AddressRetrieveSerializer(queryset)
            return Response(
    ...
    def update(self, request, pk=None):
        try:
            data = camel_to_snake_dict(request.data)
            queryset = get_object_or_404(self.get_queryset(), pk=pk)
            serializer = AddressSerializer(queryset, data=data)
            if serializer.is_valid():
                serializer.save()
                return Response(
                    snake_to_camel_dict(serializer.data),
    ...
    def partial_update(self, request, pk=None):
        try:
            data = camel_to_snake_dict(request.data)
            queryset = get_object_or_404(self.get_queryset(), pk=pk)
            serializer = AddressSerializer(queryset, data=data, partial=True)
            if serializer.is_valid():
                serializer.save()
                return Response(
                    snake_to_camel_dict(serializer.data),
    ...
    def destroy(self, request, pk=None):
        try:
            queryset = get_object_or_404(self.get_queryset(), pk=pk)
            queryset.address = uuid.uuid4() + '_deleted'
